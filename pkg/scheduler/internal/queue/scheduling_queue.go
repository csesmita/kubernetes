...
// being considered for scheduling. This is called activeQ.
...
type SchedulingQueue interface {
	...
	// Activate moves the given pods to activeQ iff they're in backoffQ.
...
// scheduling. This is called activeQ and is a Heap. The third queue holds pods
// that are moved from unschedulable queues and will be moved to active queue
// when backoff are completed.
...
func (p *PriorityQueue) activate(pod *v1.Pod) bool {
	...
	// Verify if the pod is present in backoffQ.
	// If the pod doesn't belong to backoffQ, don't activate it.
	if obj, exists, _ := p.podBackoffQ.Get(newQueuedPodInfoForLookup(pod)); !exists {
		klog.ErrorS(nil, "To-activate pod does not exist in backoffQ", "pod", klog.KObj(pod))
		return false
	} else {
